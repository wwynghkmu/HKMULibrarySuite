 ...
 *        migrate EReserve data into Alma Digital
 */

public class EReserveToAlmaDigitalOtherPDF {
			...
			for (int i = 0, j = 0; i < 50000; i++) {
				...
				if (value != null) {
					...
					if (!isMigrating.equals("N") && !isWithdraw.contains("WD") && type.toUpperCase().contains("EXAM")) {
						...
						if (!filename.equals("")) {
							if (period.equals(""))
								...
								if (type.equals("SPECEXAM"))
									destFile = period + "-" + courseCode + alSuffix + "-Specimen.pdf";
								else if (type.equals("SUPEXAM"))
									destFile = period + "-" + courseCode + alSuffix + "-Supplement.pdf";
								else
									destFile = period + "-" + courseCode + alSuffix + ".pdf";
						}
						...
						String outline = ","  + mmsid + ",,,8187250000000000,,,,,,,,,,,,,,,,,EP,,,"  + almaLabel +  ",,,"  + destFile +  ",,,";
