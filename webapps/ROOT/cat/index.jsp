...
<h1> OUHK LIB CAT Admin System </h1>
<br>
<%
session.setMaxInactiveInterval(2400);
	...
	if(!(username == null && password == null) && authen==null){
		...
		if(hk.edu.hkmu.lib.cat.Config.VALUES.get("ALLOWUSERS").toLowerCase().contains(authen.getUserid().toLowerCase()) && authen.isAuthenticated() ){
	...
	if(authen!=null && authen.isAuthenticated() && hk.edu.hkmu.lib.cat.Config.VALUES.get("ALLOWUSERS").toLowerCase().contains(authen.getUserid().toLowerCase()) ){
		response.sendRedirect("/cat/mainPage.jsp");
